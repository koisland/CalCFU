...
def load_3m_csv(input_path):
    ...
    valid_dils = df["Dilution"].isin(str(n) for n in (0, -1, -2, -3, -4))
...
def create_3m_groups(groups: pd.DataFrame, weighed: bool, plt_type: str, dils: str):
    ...
    for group in groups:
        ...
        for i, dil in zip(range(n_plates), itertools.cycle(dils)):
            ...
            plt_obj = Plate(
                plate_type=(plt["Plate Type"] if plt_type == "None" else plt_type),
...
def r_auto_results(
    ...
    """
    Read automatic input results and save as csv to read.
    """
    ...
    if int(group_n) == 0:
        ...
        df = df.assign(
            abbrID=df["Sample ID"].str.split("-").str[0],
    ...
    cfus = [
        CalCFU(grp, grp_ids).calculate()
        for grp, grp_ids in zip(plt_groups, plt_groups_ids)
    ]
...
def main():
    ...
    ap.add_argument(
        "--plate", "-p", default="None", help="Plate type (None to use recorded vals)."
    )
    ap.add_argument(
        "--dilution",
        "-d",
        default=["None"],
        nargs="*",
        help="Dilutions (None to use recorded vals).",
    )
